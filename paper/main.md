...
![图3](figures/fig5_severity_boxplot.png)
...
![图4](figures/fig8_entropy_weights.png)
...
- **电压严重度**：$f_{\text{voltage}}$ 的 $R^2\approx 1.0$（精确至0.997），几乎完美预测。这是因为电压约束严重度与新能源渗透率之间存在近似确定性的单调关系（如3.2.2节分析），GP模型仅需少量训练点即可精确捕捉这一映射。值得注意的是，该 $R^2$ 的物理含义不同于前两者——电压约束的高预测精度并非源于GP模型的强大拟合能力，而是由约束本身的确定性特征所决定。
...
![图5](figures/fig3_gp_accuracy.png)
...
![图7](figures/fig6_aia_2d_projection.png)
...
![图9](figures/fig7_transfer_limits.png)
